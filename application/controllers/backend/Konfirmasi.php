...
class Konfirmasi extends CI_Controller {
	...
	$data['konfirmasi'] = $this->db->query("SELECT * FROM konfirmasi group by kd_konfirmasi")->result_array();
	$this->load->view('backend/konfirmasi', $data);	
	}
	public function viewkonfirmasi($id=''){
	 $sqlcek = $this->db->query("SELECT * FROM konfirmasi WHERE kd_order ='".$id."'")->result_array();
	 $data['title'] = "View Konfirmasi";
	 if ($sqlcek == NULL) {
	 	$this->session->set_flashdata('message', 'swal("Kosong", "Tidak Ada Kiriman Konfirmasi", "error");');
		redirect('backend/order_tiket/vieworder/'.$id);
	 }else{		
